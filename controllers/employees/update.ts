...
import { findOrThrow, updateOrThrow404, validateAtLeastOneParamExists } from '../helpers';
...
export const updateEmployee = async ({ params: { id: uuid }, body: { companyUuid, managerUuid, name, age } }: Request) => {
  ...
  const employee = await updateOrThrow404(Employee, { uuid, name, age, company_id, manager_id });
