...
describe('throwError function', () => {
  const getError = (status: number) => { try { throwError(status); } catch (error) { return error; } };
  ...
  it('should return an error with status 422 and proper error message', () => {
    ...
    expect(error).toHaveProperty('message', { 'description': 'Unprocessable entity, missing or invalid parameters' });
  });
});
